...
export function ChatHistoryPanel() {
  const { conversationHistory, chatSessions, focusOrRestoreChat, deleteConversation, clearAllConversations } = useDashboard();
  const [query, setQuery] = useState('');
  const [selectedId, setSelectedId] = useState<string | null>(null);

  function handleOpen(record: ConversationRecord, e?: React.MouseEvent) {
    e?.stopPropagation();
    focusOrRestoreChat(record.id);
  }
  function handleDelete(record: ConversationRecord, e?: React.MouseEvent) {
    e?.stopPropagation();
    if (!confirm(`Delete "${record.title}"?\nThis removes the conversation from local history and from the cloud archive. Cannot be undone.`)) return;
    deleteConversation(record.id);
    if (selectedId === record.id) setSelectedId(null);
  }
  function handleClearAll() {
    if (!confirm('Wipe ALL chat history?\nDeletes every conversation locally and in the cloud archive, and clears every open chat session. Cannot be undone.')) return;
    ...
    <div className="chat-history">
      <div className="chat-history__sidebar">
        <div className="chat-history__search-row">
          ...
          <button
            className="chat-history__clear-all"
            onClick={handleClearAll}
            disabled={records.length === 0}
            title="Delete every conversation locally + in the cloud archive"
          >
            Clear all
          </button>
        </div>
        <div className="chat-history__list">
          {filtered.map(record => (
            <div
              ...
              role="button"
              tabIndex={0}
              onKeyDown={e => { if (e.key === 'Enter') setSelectedId(record.id); }}
            >
              ...
              <div className="chat-history__item-actions">
                <button
                  className="chat-history__item-action"
                  onClick={e => handleOpen(record, e)}
                  title="Open / restore this conversation in a chat window"
                >
                  Open
                </button>
                <button
                  className="chat-history__item-action chat-history__item-action--danger"
          ...
          <>
            <div className="chat-history__detail-head">
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                <button className="chat-history__export" onClick={() => handleOpen(selected)} title="Open / restore this conversation">
                  Open
                </button>
                <button className="chat-history__export" onClick={() => exportConversation(selected)}>
                  Export
                </button>
                <button className="chat-history__export chat-history__export--danger" onClick={() => handleDelete(selected)}>
                  Delete
                </button>
              </div>
            </div>
